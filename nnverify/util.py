...
def get_torch_net(net_file, dataset, device='cpu'):
    ...
    elif dataset == Dataset.AAAI:
        model = models.Models['acasxu']([5,50,50,50,50,50,50,5])
    else:
    ...
    if 'kw' in net_file:
        ...
        model.load_state_dict(torch.load(net_file, map_location=torch.device(device))['state_dict'][0])
    elif 'perturb_rate' in net_file:
        model.load_state_dict(torch.load(net_file, map_location=torch.device(device)))
    else:
